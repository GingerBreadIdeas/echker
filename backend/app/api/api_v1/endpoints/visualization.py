...
from bokeh.models import ColumnDataSource, DataTable, TableColumn, HoverTool, Circle, MultiLine, CDSView, BooleanFilter
...
def get_message_embeddings_data(
    ...
    try:
        ...
        logger.info(f"Successfully reduced embeddings to 2D with shape {points_2d.shape}")
...
def get_message_embeddings_html(
    ...
    p.circle('x', 'y', source=source, view=regular_view, size=8, color='#0074D9', alpha=0.7, legend_label="Regular Messages")
    p.circle('x', 'y', source=source, view=injection_view, size=10, color='#FF4136', alpha=0.7, legend_label="Prompt Injection")
